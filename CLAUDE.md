...
**바로 시작하지 않는다.** 계획을 양식에 맞춰 먼저 제시하고, 승인을 받은 뒤 진행한다.
...
- **폴더별 `CLAUDE.md`와 에이전트 정의에는 `docs/` 경로를 적지 않는다.** 작업마다 달라지는 문서를 두 군데가 지시하면 무엇이 컨텍스트에 올라올지 예측할 수 없다. 이 문서들은 항상 참인 것만 담는다.
