...
| 영속성 (엔티티) | Spring Data JPA + Hibernate 7 | 32개 테이블 매핑, 모든 변경 경로 |
...
| Flyway | — | 32개 테이블 스키마를 형상 관리 대상으로 둔다. DDL을 단일 진실 원천으로 고정해야 엔티티가 설계서에서 이탈하지 않는다. `hibernate.ddl-auto`는 `validate`로 고정한다. **롤링 배포 중 신·구 버전이 공존하므로 마이그레이션은 하위 호환이어야 한다** |
...
| Flyway 마이그레이션 | — | `V1__init_schema.sql`에 초기 30개 테이블 DDL을 정의하고(이후 테이블은 다음 번호 마이그레이션), 시드 데이터(보증보험 3사 기준, 대출 규제 수치, 공통 코드)는 별도 마이그레이션으로 분리한다 |
