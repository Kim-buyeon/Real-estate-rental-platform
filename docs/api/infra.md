...
**INF-05 행은 미구현이다** — 관측 작업(로드맵 6주차)에서 구현한다. 2026-09-19에 차기 범위로 뺐다가 2026-09-23에 범위로 돌아왔다. 행은 자체 호스팅 관측 스택을 전제로 썼으므로 관측 작업에서 노드 밖 전송 기준으로 다시 본다. 지금 구현된 것은 INF-01 readiness 하나다.
