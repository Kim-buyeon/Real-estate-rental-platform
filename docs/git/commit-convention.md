...
| `infra/grafana/` · `infra/alertmanager/` · `infra/promtail/` | 알림 규칙, 대시보드 — 자체 호스팅으로 되돌릴 때의 자리. 지금은 없다 |
...
| `smoke.sh` 기준 + `docs/infra/runbook.md` 3.3의 기준 표 | 판정 기준(실패 1% · p95 1초 · 60건)이 스크립트와 절차서 양쪽에 나온다. 관측을 세우면 알림 임계도 이 짝에 들어간다 — 배포 판정이 알림보다 느슨하면 배포는 통과했는데 직후에 알림이 울린다 |
