...
**관리형 서비스에 의존하지 않는다** — 모든 구성 요소를 직접 기동한다. 서비스 구성 요소는 컨테이너로, OS 기능(NFS 서버 · NAT · 방화벽 · 정기 작업)은 Rocky 표준 패키지로 둔다. 클라우드 사업자의 관리형 데이터베이스·로드밸런서·모니터링에 의존하지 않으므로 온프레미스나 프라이빗 클라우드로 그대로 이식할 수 있다.
...
Rocky Linux로 옮긴 뒤 더하는 서버 운영 기반(INF-07 ~ 09)의 기술이다. **설계와 근거는 서버 운영 기반 설계서(`docs/infra/platform.md`)가 갖는다** — 여기에는 선정 결과만 둔다.
