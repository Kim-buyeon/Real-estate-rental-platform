...
| 슬롯마다(APP-02 app-2 → APP-01 app-2 → APP-02 app-1 → APP-01 app-1) | `down` 토글 → drain 30초 → 교체(`--no-deps --force-recreate` — APP-02 슬롯은 SSH로 그 노드의 체크아웃에서) → readiness 최대 120초(APP-01에서 그 슬롯의 주소로) | 그 슬롯을 `down`으로 둔 채 멈춘다. 남은 슬롯이 구버전으로 서비스한다 |
