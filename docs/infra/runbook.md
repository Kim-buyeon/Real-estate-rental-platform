...
| 월 1회 | 커널 갱신 반영을 위한 계획 재부팅(한 노드씩 — 노드별 영향은 표 아래), 계정 목록 대조(전 노드 동일 — 9.2절), 백업 복원 검증(물리 · 논리 각각), 가용성 목표 달성률 집계(`docs/infra/system.md` 5.1절), 용량 추이 대비 증설 시점 판단, 외부 API 호출량·비용 집계 |
...
| 노드 | 재부팅 중 | 방법 |
|---|---|---|
| APP-01 | **서비스 전체 중단**(단일 노드 — 두 슬롯 · Nginx가 함께 멈춘다, 시스템 구성서 3장) | 공지 후 저트래픽 시간대. 기동 뒤 배포 확인(3장)과 같은 확인 |
...
**모니터링이 없으므로(INF-05 차기) 점검은 사람이 확인하는 것이다.** 위 표의 확인을 거르면 디스크가 차거나 백업이 멈춰도 알 수 없다. 외부 경로 감시(HetrixTools) 일시정지 규칙은 관측이 돌아올 때 되살린다.
...
**논리 백업 정기 작업** — `infra/backup/pg-dump.sh`를 systemd timer(`rental-backup.timer` · 매일 02:00)가 부른다. 순서는 primary 확인 → `pg_dump -Fc` → 암호화 → 목적지 적재 → 보존 기간 지난 것 삭제이고, standby에서는 아무것도 하지 않고 끝난다. **왜 그렇게 두는지는 서버 운영 기반 설계서 5.1 · 7.2가 갖는다.** 여기에는 실행할 때 확인할 것만 적는다.
...
**아직 정해지지 않은 것** — 목적지(지금은 노드의 로컬 경로, NAS는 INF-08 이후) · 보존 기간(잠정 7일) · 암호화 도구 · 실행 시간 상한(잠정 30분). 값과 확정 시점은 서버 운영 기반 설계서 5.4 · 10장이 갖는다. **선행 조건이 하나 있다** — 지금 PostgreSQL은 APP-01의 컨테이너이고 호스트 포트를 열지 않아 호스트 클라이언트가 닿지 않는다. 루프백 게시(같은 설계서 10장 「5432 호스트 규칙(DOCKER-USER)」)가 정해져야 이 작업을 노드에 넣을 수 있다.
...
| 용량 | 디스크 사용률 추이(노드별 데이터 볼륨 · NAS), 증설 판단 | 2장 일일 점검 기록 — 대시보드는 관측(차기)과 함께 |
...
서버 운영 기반(INF-07 ~ 09)의 손으로 밟는 순서다. **무엇을 왜 그렇게 두는지는 서버 운영 기반 설계서(`docs/infra/platform.md`)가 갖는다.** 명령은 첫 구축에서 실제로 실행하며 이 절에 채운다(1장 원칙) — 지금은 단계와 확인 기준만 있다.
...
| 2 | 데이터 볼륨 부착 → XFS 생성 → UUID + `nofail`로 fstab → 재부팅 | 재부팅 뒤 마운트 유지 |
| 3 | 운영자 개인 계정 · `deploy` · `backup`(고정 UID · GID) 생성. `rocky`는 `authorized_keys` 제거 · 계정 만료 · `nologin` — `passwd -l`만으로는 공개키 로그인이 막히지 않는다(설계서 6.2) | `rocky`로 SSH 시도가 거부된다 |
...
| 수동 1회 실행 | `systemctl start rental-backup.service` → `journalctl -u rental-backup -n 50`. 첫 실행은 이렇게 확인하고 소요 시간을 재 실행 시간 상한을 확정한다 |
