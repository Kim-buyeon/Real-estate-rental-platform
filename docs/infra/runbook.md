...
**열린 포트는 밖에서 본다.** 노드 안에서 `ss`로 보는 것과 인터넷에서 닿는 것은 다르다 — Docker가 게시한 포트는 호스트 방화벽을 우회하므로(설계서 3.3) 루프백 바인딩이 실제로 먹는지는 외부에서 두드려야 판정된다. 시스템 구성서 4장의 허용 표와 대조해 **표에 없는 포트가 열려 있으면 그것이 결함이다.** 특히 데이터 저장소(5432 · 6379 · Redis TLS 6380)가 닿으면 즉시 조치한다. 운영자 접속(22)의 출발지가 표대로 좁혀져 있는지도 함께 본다.
...
**알림까지 걸리는 시간은 끊긴 방식에 따라 다르다**(2026-09-26 · #253 · #255 실측, 표본 1회씩).

| 끊긴 방식 | 알림까지 | 실측 |
|---|---|---|
| 수집 대상 정지 — 시계열이 남아 `up = 0` | 약 5 ~ 6분(대기 5분) | DB-02 node exporter 14:54:04 정지 → 15:00:00 Firing |
| 노드 통째 정지 — 시계열이 사라져 `absent` | **약 10분** — 사라졌다고 판정되기까지 약 5분 + 대기 5분 | APP-02 인스턴스 15:45:56 정지 → `absent(up{node="app-02", job="node"})` **15:51:00 Pending** → (15:56 Firing 예정) 15:54:30 다시 켬 → 15:57:00 Normal |

사라짐 판정의 약 5분은 **질의의 기본 조회 구간(lookback) 5분 때문으로 보인다 — 추정이다.** 줄일 수단(대기 단축 등)은 정하지 않았다 — 후속으로 남긴다.
...
| 슬롯마다(APP-02 app-2 → APP-01 app-2 → APP-02 app-1 → APP-01 app-1) | `down` 토글 → drain 30초 → 교체(`--no-deps --force-recreate` — APP-02 슬롯은 SSH로 그 노드의 체크아웃에서) → readiness 60회 × (시도 + 2초) — 루프백처럼 연결이 즉시 거절되면 약 120초, 원격 슬롯이 패킷을 버리면(연결 상한 2초) 약 240초, 연결 뒤 응답이 늦으면(요청 상한 3초) 약 300초(APP-01에서 그 슬롯의 주소로) | 그 슬롯을 `down`으로 둔 채 멈춘다. 남은 슬롯이 구버전으로 서비스한다 |
...
| 순서 | 하는 일 |
|---|---|
| 1 | 출력에서 `down`으로 남은 슬롯(노드 · 이름)과 차이를 확인한다. 직전 배포가 왜 멈췄는지 먼저 안다 |
...
```bash
bash smoke.sh [<주소>] <포트>    # 통과 0, 이상 1. 주소를 빼면 127.0.0.1(APP-01 슬롯)
```

APP-02 슬롯은 그 노드의 사설 IP에 게시되므로 주소를 준다 — 배포 스크립트는 슬롯마다 주소와 포트를 넘긴다(#255).
...
- 롤백은 대상 태그를 지정해 같은 스크립트를 다시 실행하는 것으로 갈음한다. **이때 관찰 단계는 건너뛴다** — 되돌아갈 대상이 이미 검증된 버전이기 때문이다. 소요는 슬롯 둘일 때 약 3분이었다(drain 30초 × 2 + 기동·확인). **슬롯 넷의 일상 배포는 약 5분이었다**(관찰 포함 — 3.2 4슬롯 배포 리허설, #255). 롤백은 첫 슬롯의 관찰(워밍업 60초 + 판정)을 건너뛰어 그보다 짧다 — **롤백 자체는 4슬롯에서 재지 않았다**
...
**값은 설정 파일이 갖는다.** 여기에 옮겨 적지 않는다 — 두 곳에 적으면 한쪽만 고쳐진다. 서버 문맥에는 이 밖에 요청 본문 크기 상한과 헤더 · 본문 수신 타임아웃이 있고, 업스트림으로 **요청 추적 ID를 전달**한다(접근 로그의 `rid`와 같은 값이며, 앱은 이것을 로그의 `traceId`로 쓴다). 슬롯이 넷(두 노드)이라 이 키가 없으면 접근 로그와 앱 로그를 이을 수단이 없다.
...
**클라이언트(IP)별 요청 상한**(2026-09-26 · #266 — 값은 `default.conf` 머리). 아래 전체 상한은 키가 하나라 클라이언트 하나가 몫을 다 쓰면 모두가 429를 받는다 — 그래서 IP별 상한을 함께 건다. 두 상한 중 먼저 닿는 쪽이 429다. 통신사 공유 IP 뒤의 여러 사용자를 한 클라이언트로 보므로 넉넉한 잠정값이다. **부하 생성기 LOAD-01 주소는 IP별 상한에서 뺀다**(전체 상한은 받는다) — 한 주소에서 수백 건/초를 내야 하기 때문이다.

**429 비율 보기** — 앞단 exporter(`stub_status`)는 상태 코드를 세지 않는다. 앞단 접근 로그가 Loki로 가므로(Vector) Grafana 탐색에서 LogQL로 본다: `sum(count_over_time({node="app-01", service="nginx"} |= "\" 429 " [5m])) / sum(count_over_time({node="app-01", service="nginx"} |= "/api/" [5m]))`. 라벨(`node` · `service`)은 Vector의 `sinks.loki.labels`가 붙인다. **알림 권장**: 5분 비율 1% 초과 10분 지속 — 상한에 닿기 시작했다는 뜻이라 증설 · 개선 시점의 선행 지표다(용량 산정 리포트). 알림 규칙은 사람이 Grafana에서 만든다(2장 알림 규칙 표에 더한다).
...
**접속 대상 전환** — 앱의 접속 대상은 `infra/.env`의 `DB_HOST`다. **APP-02(#255)가 있으면 그 노드의 `.env`에도 같은 줄이 있다** — 아래 6.3의 `DB_HOST` 단계는 APP-01만 적었으므로 APP-02의 `.env`도 함께 바꾸고 그 노드의 두 슬롯도 재생성한다. 4노드에서 밟아 보지 않았다(미검증). 단일 노드는 평시에 두지 않는다(비우면 `postgres`) · **3노드는 평시 `DB_HOST=10.20.10.10`**(6.3). 6.1 6단계에서 `DB_HOST=postgres-standby`를 넣고 슬롯을 재생성하며, 페일백 7단계에서 그 줄을 지우고 다시 재생성한다(#199).
...
서버 운영 기반(INF-07 ~ 09)의 손으로 밟는 순서다. **무엇을 왜 그렇게 두는지는 서버 운영 기반 설계서(`docs/infra/platform.md`)가 갖는다.** 명령은 첫 구축에서 실제로 실행하며 이 절에 채운다(1장 원칙) — 지금 명령까지 채워진 것은 9.1의 AL2023 OS 보안 기준 · 9.1의 Rocky 3노드 준비 · 9.1의 앱 노드 APP-02 준비 · 9.2의 계정 추가 · 9.3의 논리 · 물리 백업 · NFS 공유이고 나머지는 단계와 확인 기준만 있다.
...
| 순서 | 명령 | 확인 · 실측 |
|---|---|---|
| 1 | 보안 그룹 `rental-app-node`(22 · 8081-8082 ← `rental-app`) → `rental-app`에 6380 ← `rental-app-node`, `rental-db`에 5432 ← `rental-app-node` → `run-instances`(t3.small · 같은 AMI · 2c 사설 서브넷 · 사설 IP `10.20.20.30` · **공인 IP 없음** · `HttpTokens=required,HttpPutResponseHopLimit=1` · 루트 20 + 데이터 20 GB gp3 `Encrypted` · `Project=rental`) | `i-0b2e25a39a9b20d00`. 출구는 APP-01 NAT |
| 2 | 3노드 표 2 ~ 12 — 시간대 · 데이터 볼륨 `/var/lib/docker`(XFS · UUID · `nofail`) · sshd · journald · 계정 · firewalld(`infra/os/firewalld/app-node.sh` — ssh만) · `dnf-automatic` · Docker, 그리고 kdump 끄기(위 메모리 단락) · 재부팅. `git`도 설치한다 | Docker 29.8.1 · Compose v5.5.1, 재부팅 뒤 `free` total **1905 MiB** |
| 2 함정 | **`dnf config-manager`는 `dnf-plugins-core`가 없으면 실패한다** — Rocky 기본 이미지에 없다. 그 패키지를 먼저 설치한다 | — |
| 2 계정 | `backup`(2001) → `buyeon`(2002) → `deploy`(2003) | **`deploy`의 기본 그룹 GID가 2004, `docker` 그룹이 2003이다** — `docker` 그룹을 계정보다 먼저 만들었다. 다른 노드와 GID가 다를 수 있다 — 확인하지 않았다(설계서 6.2 APP-02 적용 상태) |
| 3 | 체크아웃 — `deploy`로 공개 저장소를 https로 `git clone` → `/home/deploy/rental` | — |
| 4 | **Redis TLS 인증서(APP-01)** — `/etc/rental/redis-tls/`에서 `sudo openssl req -x509 -nodes -newkey ec -pkeyopt ec_paramgen_curve:prime256v1 -keyout server.key -out server.crt -days 825 -subj "/CN=app-01.rental.internal" -addext "subjectAltName=IP:10.20.0.10"` → `ca.crt`는 `server.crt`의 사본(자체 서명이라 서버 인증서가 곧 CA) → 셋 다 소유 `999:999`, 키 600. **APP-02에는 `ca.crt`만** `/etc/rental/redis-tls/ca.crt`로 | 만료 **2028-12-29**. 서버 개인키는 APP-02에 가지 않는다 |
| 5 | `.env` — APP-01 `.env`에 `REDIS_TLS=on` · `REDIS_TLS_PUBLISH_ADDR=10.20.0.10`. APP-02 `.env`는 **APP-01 `.env`의 사본**에 `COMPOSE_PROFILES=app-node` · `APP_PUBLISH_ADDR=10.20.20.30` · `APP_REDIS_HOST=10.20.0.10` · `APP_REDIS_PORT=6380` · `RENTAL_NODE=app-02` · `REDIS_TLS_CA_DIR=/etc/rental/redis-tls` · `SPRING_DATA_REDIS_SSL_ENABLED=true` · `SPRING_DATA_REDIS_SSL_BUNDLE=redis` · `SPRING_SSL_BUNDLE_PEM_REDIS_TRUSTSTORE_CERTIFICATE=file:/etc/rental/redis-tls/ca.crt`. 변수의 뜻은 루트 `.env.example` | APP-02 `.env`는 DB 노드처럼 줄이지 않았다 |
| 6 | **PostgreSQL `max_connections` 32 → 48**(슬롯 넷의 풀 — 산정은 운영 Compose 주석) — **standby 먼저**: DB-02 `postgres-standby` 재생성 → 그다음 DB-01 `postgres` 재생성. 거꾸로 하면 standby가 복구를 멈춘다(운영 Compose의 `max_connections` 주석) | DB-02 15:41:14 → 15:41:22, DB-01 15:41:31 → healthy 15:41:37.995, 복제 `streaming` 복귀. 재생성 직후 API 6건 200 |
| 7 | **배포 신뢰 경로** — APP-01 `deploy`에 전용 키 `ssh-keygen -t ed25519 -N '' -f ~/.ssh/app-node_ed25519`(비밀구절 없음 — 자동화) → 공개키를 APP-02 `deploy`의 `authorized_keys`에 **`from="10.20.0.10"`을 앞에 붙여** 한 줄로 → APP-01 `deploy`의 `~/.ssh/config`(600)에 `Host 10.20.20.30` · `User deploy` · `IdentityFile ~/.ssh/app-node_ed25519` · `IdentitiesOnly yes`. 배포 스크립트의 SSH는 `-i`를 주지 않고 비대화식이라 이 설정과 APP-02의 호스트 키(`known_hosts`)가 있어야 한다 | **SSH 설정이 없던 첫 배포 리허설(16:00:39)은 「원격 노드 확인」에서 `Permission denied (publickey)`로 아무 슬롯도 건드리지 않고 멈췄다** — 스크립트의 안전장치가 설계대로 동작한 실측이다 |
| 8 | 반영 — APP-01 `redis` 재생성(4 · 5로 6380을 연다), APP-02 `docker compose up -d`(프로필 `app-node` — 슬롯 둘 · node exporter · `prom-agent-app-node` · `vector`), APP-01 `upstream.conf` 체크아웃 → `nginx -t` → reload(3.1). 이 셋의 시각은 기록하지 않았다 | Nginx 4슬롯 반영 뒤 40건 전부 200, 슬롯별 16 / 14 / 9 / 9 |

**리허설 실측**(2026-09-26, #255 — 표본 1회씩)

| 확인 | 방법 | 결과 |
|---|---|---|
| APP-02 노드 정지 | **APP-02 인스턴스 정지**(`stop-instances` 명령 15:45:56) 동안 밖에서 0.1초 간격 요청 | **595건 전부 200.** 다만 15:46:02부터 **36건(약 6%)이 2 ~ 4초** — `max_fails`가 10초마다 원격 슬롯을 다시 시도하고 연결 시간 초과가 2초라서다. 두 원격을 연달아 고르면 4초(재시도 상한은 `default.conf`의 `/api/` 블록) |
...
| SSE 노드 간 팬아웃 | — | **미검증** — 알림 이벤트를 만들 수단이 없었다. Redis 공유는 노드 간 토큰으로 확인했다 |
| 4슬롯 배포 | `bash deploy.sh <태그>`(같은 태그 재배포) | 약 5분, 배포 중 조회 1,369건 전부 200 — 3.2 |
...
**노드 지표(3노드 · APP-02)** — **모든 노드** `sudo install -d -o backup -g backup -m 755 /var/lib/rental-metrics`(APP-02는 준비 때 빠뜨려 `node_textfile_scrape_error 1`이 났다가 2026-09-26 16:1x에 만들어 0 — #255)(없으면 node exporter가 `node_textfile_scrape_error 1`과 스크레이프마다 오류 로그를 낸다 — APP-01에서 실측)(정기 작업이 결과 지표를 쓰는 자리 — node exporter가 `/`를 `/host`로 붙여 읽는다) → DB 노드 `.env`에 `RENTAL_NODE=db-01`(DB-02는 `db-02`) → 스크립트 재설치 → `docker compose up -d`(프로필 `db` · `standby`가 `node-exporter` · `prom-agent-db`를 올린다). 확인 — `curl -s 127.0.0.1:9100/metrics | grep ^rental_`, `node_textfile_scrape_error 0`, 수집기 `127.0.0.1:9090/metrics`의 `prometheus_remote_storage_samples_failed_total 0`(#243). **단일 노드 구성(`app,db,standby`)에서는 `prom-agent-db`를 올리지 않는다** — `prom-agent`와 9090이 겹친다(운영 Compose 주석)
