...
| 3 | 구 primary 격리 — 살아 있으면 정지하고 다시 뜨지 않게 한다(같은 노드: `docker compose stop postgres`) | 1분 | 스플릿 브레인 방지. 구 primary가 쓰기를 받을 길이 없다 |
...
추정 20분 대비 서비스 복귀 **77.6초**다. 차이는 사람의 판단 · 알림 시간이 빠진 것과 데이터가 작은 것(36 MB)에서 온다. **남은 시간의 대부분은 슬롯 재기동**이므로, 줄이려면 접속 대상을 재기동 없이 바꾸는 수단(앞단 프록시 · DNS 별칭)을 봐야 한다.
