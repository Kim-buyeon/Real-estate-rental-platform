...
> ※ 설계 근거는 `docs/features/infra.md`를, 서버 · 네트워크 · 계정 · 정기 작업의 설계는 `docs/infra/platform.md`를 따른다. 지표와 알림은 `docs/infra/observability.md`를 따른다 — 미작성이다. 그 전까지 지표 이름은 인프라 API 명세 1장, 알림 규칙은 2장이 갖는다.
> 작성 기준일 : 2026년 8월
...
| 주 1회 | 전체 백업 성공 여부, 논리 백업의 S3 사본 생성 여부(3노드 — `aws s3 ls`는 운영자 자격 증명으로, 9.3절), 복제 지연 추이, 슬로우 쿼리 상위 10건, 캐시 적중률 추이 |
| 월 1회 | 커널 · 보안 갱신 반영을 위한 계획 재부팅(한 노드씩 — 3노드는 `dnf-automatic`이 받은 갱신을 반영한다. 노드별 영향과 옛 AL2023 노드의 갱신 명령은 표 아래), 계정 목록 대조(전 노드 동일 — 9.2절), 백업 복원 검증(물리 · 논리 각각), 가용성 목표 달성률 집계(`docs/infra/system.md` 5.1절), 용량 추이 대비 증설 시점 판단, 외부 API 호출량·비용 집계, **밖에서 본 열린 포트 확인**(아래) |
...
**월 주기로 둔 이유는 1차 배포 구간 때문이다.** 그 구간(옛 AL2023 노드)은 호스트 방화벽 없이 두 겹으로 돌았다(설계서 3.3). Rocky 이전까지가 짧아 분기 주기로는 그 안에서 한 번도 돌지 않고 끝날 수 있었다 — 두 겹뿐인 동안을 덮으려고 만든 확인이 그 동안을 덮지 못한다. **3노드는 firewalld까지 세 겹이 갖춰졌다**(2026-09-25 · #233) — 주기는 다시 본다.

**3노드(Rocky)는 `dnf-automatic`이 보안 갱신을 매일 받고, 월 1회 계획 재부팅이 커널 갱신을 반영한다**(설계서 7.2 · 8장 패키지 갱신 행). 재부팅 전에 사람이 올릴 것은 없다.

**옛 AL2023 노드(점검 모드로 남긴 노드)는 재부팅 전에 갱신을 사람이 올린다.** `dnf-automatic`이 새 보안 갱신을 받지 못하기 때문이다(설계서 8장 패키지 갱신 행). `sudo dnf check-update --releasever=latest --security`로 대상을 보고, 그 시점의 날짜 박은 버전(`dnf check-release-update`가 알려 준다)으로 `sudo dnf upgrade --releasever=<버전>`을 실행한 뒤 재부팅한다. `latest`를 그대로 쓰지 않는 것은 같은 버전을 다른 노드 · 다음 노드에 다시 밟을 수 있게 하려는 것이다. Docker · containerd가 함께 올라오면 기동 뒤 배포 확인(3장)을 빠뜨리지 않는다.

**계획 재부팅의 노드별 영향** — 서비스가 끊기는 노드는 트래픽이 낮은 시간대에 미리 알리고 한다.

| 노드 | 재부팅 중 | 방법 |
|---|---|---|
| APP-01 | **서비스 전체 중단**(앱 계층이 이 노드 하나 — 두 슬롯 · Nginx가 함께 멈춘다, 시스템 구성서 3장). **3노드에서는 NAT을 겸하므로 DB 노드의 인터넷 출구도 함께 끊긴다**(시스템 구성서 4장). **3노드 실측 — 재부팅 명령에서 API 200까지 92초**(2026-09-25, 9.5 9단계). 이하 옛 단일 노드 실측 — **부팅 후 전 컨테이너 정상 88초**(2026-09-23). 화면은 10초쯤에 뜨지만 그때 API 는 아직 실패한다 — **복귀 판정은 API 로 한다**(71초). 2026-09-25 재부팅(SELinux enforcing 전환 확인)은 명령에서 API 첫 200까지 약 93초(9.1) | 공지 후 저트래픽 시간대. 기동 뒤 배포 확인(3장)과 같은 확인 |
...
**알림 규칙 — Grafana Cloud(`rental` 폴더 · `rental-backup` 그룹)에 걸려 있다**(2026-09-26, #253). Grafana 화면(로그인한 사람)에서 건다 — 노드의 수집 토큰은 쓰기 전용이라 규칙을 만들 수 없다(#243). 평가 1분 · 대기(`for`) 5분 · 연락처 `bu2000kim`(메일). 지표 이름은 인프라 API 명세 1장.
...
| 수집 끊김 | `up < 1` 또는 노드별 `absent(up{node="app-01", job="node"}) * 0`(db-01 · db-02도 같은 모양) — 임계 `< 1`, 데이터 없음 → OK | 노드 · 수집기가 멈추면 위 규칙도 침묵한다 |

**`up == 0` 하나로는 울리지 않는다.** 결과 값이 0이라 임계를 `> 0`으로 두면 늘 거짓이다. 식이 `up`이어도 노드가 통째로 꺼지면 그 노드의 시계열이 사라질 뿐 0이 되지 않고, Grafana는 사라진 시계열의 알림을 해소로 처리한다(2026-09-26 14:47 규칙 이력의 `Normal (MissingSeries)`로 관찰). 그래서 노드 이름을 박은 `absent`를 더한다(2026-09-26 DB-02 node exporter를 14:54:04에 멈춰 15:00:00 Firing · 메일 발송을 확인, #253). 노드를 늘리면 이 식에 한 줄을 더한다.

**옛 노드(학교 계정 3.36.74.44)가 같은 `node="app-01"` 라벨로 지표를 보내는 동안 app-01의 `absent`는 울리지 않는다** — 새 APP-01이 꺼져도 옛 노드의 `up`이 남는다. 대시보드의 app-01 패널도 두 노드가 섞인다. 옛 노드의 수집기(prom-agent · vector) 정지가 남았다(#253).
...
| **첫 슬롯은 복귀 전에 관찰한다** | 워밍업 60초 뒤 교체한 슬롯을 직접 두드려(3.3) 통과해야 upstream에 넣는다. 배포 판정에 쓸 알림 규칙(오류율 · 지연)이 없어 지표 대신 판정 요청을 스스로 만든다 |
...
**배포 판정에 쓸 알림 규칙(오류율 · 지연)이 없어 지표를 조회하지 않는다**(INF-05 — 2장의 규칙은 백업 · WAL · 수집 끊김뿐이다). 2026-09-19에 차기 범위로 뺐다가 2026-09-23에 범위로 돌아왔다 — 오류율 · 지연 규칙을 걸면 이 절을 다시 본다. 대신 교체한 첫 슬롯을 **upstream에 넣기 전에** 직접 두드린다. 신버전이 사용자 요청을 받기 전에 걸러진다.
...
**논리 백업(`pg_dump -Fc`, INF-08 — 3노드는 DB 노드 로컬 + S3 `logical/`, 옛 단일 노드는 NAS)은 하루 한 번이다** — 되살리면 최대 약 하루 전 상태다. 그래서 테이블 단위 복원에 쓰는 것은 **그 테이블이 백업 뒤 바뀌지 않았을 때**(기준 데이터 · 적재 뒤 고정된 데이터)와 다른 버전 · 서버로 옮길 때뿐이다. 그 밖의 손상은 위 순서대로 PITR로 직전 시각을 복원한 뒤 필요한 테이블만 추출한다 — 1등급 RPO 5분(시스템 구성서 5.1)은 물리 백업 + WAL이 지킨다. 논리 복원 순서는 9.3절.
...
**전송이 한 파일에서 멈추면 그 뒤로는 아무것도 보내지 않는다** — 첫 실패에서 끝나고 매분 같은 실패를 저널에 남긴다(0600 세그먼트에서 실측). 그동안 노드 소실 RPO는 5분이 아니라 **멈춘 시간만큼** 늘고, 멈춘 채 주간 물리 백업이 돌면 `pg_archivecleanup`이 아직 보내지 않은 세그먼트를 지워 S3 사슬에 구멍이 난다. **이것은 알림 「WAL 전송 멈춤」 · 「WAL 전송 밀림」이 메일로 알린다**(2장, 2026-09-26 #253) — 알림을 받으면 저널에서 멈춘 파일을 찾는다.
...
| 확인 | 내용 |
|---|---|
| 접속 | 호스트의 PostgreSQL 클라이언트가 `PGHOST` · `PGPORT` · `PGUSER` · `PGDATABASE`로 붙는다. 컨테이너 안의 `pg_dump`를 부르지 않는다 — 정기 작업은 `docker` 그룹이 아닌 `backup` 계정으로 돌고(설계서 6.2), 3노드는 `PGHOST`가 그 노드의 사설 IP다(9.3 「3노드」 2단계) |
...
**아직 정해지지 않은 것** — 보존 기간. 잠정값(7일)은 스크립트가 갖고, 확정 시점은 서버 운영 기반 설계서 5.4가 갖는다. 실행 시간 상한은 30분으로 확정했다(같은 설계서 5.3). 암호화 도구(`gpg`)와 `backup` 계정 UID · GID는 같은 설계서 5.1 · 6.2가 정했다. 호스트 클라이언트가 닿도록 운영 Compose가 PostgreSQL을 게시한다 — 3노드는 그 노드의 사설 IP(위 「3노드의 정기 백업」), 옛 단일 노드는 루프백(`127.0.0.1:5432`)이다(시스템 구성서 4장).
...
**물리 백업 정기 작업** — `infra/backup/pg-basebackup.sh`를 `rental-basebackup.timer`가 주 1회 부른다(시각은 서버 운영 기반 설계서 7.2). 순서는 primary 확인 → `pg_basebackup -Ft -z`(WAL 포함) → **`BASEBACKUP_S3_URI`가 있으면 세 파일을 암호화해 S3 `physical/<시각>/`로**(실패하면 로컬 보존 · WAL 정리는 마치고 종료 1) → 최신 4개 보존 → 가장 오래 남긴 백업의 시작 WAL보다 앞선 아카이브를 `pg_archivecleanup`으로 정리. 접속 · 비밀번호 규칙은 위 논리 백업 표와 같고 역할만 다르다 — 복제 권한을 가진 전용 역할 `rental_basebackup`(`EnvironmentFile`은 `/etc/rental/basebackup.env`). 호스트 도구는 3노드(Rocky)에서 PGDG `postgresql17` · `postgresql17-server`에 있다 — `pg_archivecleanup`은 `-server`에 있고 `/usr/pgsql-17/bin`에만 있다(9.3 「3노드」 1단계). 옛 AL2023 노드에서는 `postgresql17-server`(`pg_basebackup`) · `postgresql17-contrib`(`pg_archivecleanup`)이다. 설치해도 호스트의 `postgresql.service`는 켜지 않는다.
...
**옛 단일 노드에서 standby는 같은 노드의 컨테이너(`postgres-standby`)였다**(3노드에서는 DB-02 — 6.3)(시스템 구성서 2.2 · 3장). 아래 절차의 「primary」는 `postgres` 서비스, 「standby」는 `postgres-standby` 서비스로 읽는다. 노드가 통째로 멈추면 둘이 함께 멈추므로 1단계의 「SSH 접속 불가」 판정은 이 구성에서 성립하지 않는다 — **primary 컨테이너만 멈춘 경우의 절차 검증**이다. 명령은 로드맵 7주차 실행에서 이 절에 채운다(1장 원칙).
...
**접속 대상 전환** — 앱의 접속 대상은 `infra/.env`의 `DB_HOST`다. 단일 노드는 평시에 두지 않는다(비우면 `postgres`) · **3노드는 평시 `DB_HOST=10.20.10.10`**(6.3). 6.1 6단계에서 `DB_HOST=postgres-standby`를 넣고 슬롯을 재생성하며, 페일백 7단계에서 그 줄을 지우고 다시 재생성한다(#199).
...
페일오버가 끝난 상태는 **정상 구성이 아니다.** standby가 primary 역할을 하고 있고, 그동안 standby가 없다. 옛 설계는 standby를 원래 primary보다 낮은 사양으로 두어 처리 여력도 줄었다 — **3노드는 두 DB 노드가 같은 사양(t3.small)이라 줄어드는 것은 이중화다.** 서비스는 살아 있지만 축소된 상태이므로, 원래 구성으로 되돌리는 절차를 함께 정의한다.
...
**단일 노드에서는 페일오버가 끝난 동안 논리 백업이 멈춘다**(3노드는 새 primary가 이어받는다 — 6.3). 백업은 `postgres` 서비스가 루프백에 게시한 5432로 붙는데(5장) `postgres-standby`는 단일 노드에서 루프백 5433에 게시해 백업 경로(5432)에 없다. 그 상태를 오래 두지 않는다.
...
**되돌리지 않는 선택지도 있다.** 승격된 노드를 계속 primary로 두고 구 primary를 standby로 붙이는 방식이다. 전환 작업이 한 번 줄어드는 대신, 두 노드의 사양이 다르면(옛 설계) 뒤바뀐 채로 남으므로 사양을 맞추는 작업이 따로 필요하다. 어느 쪽을 택하든 **사양이 낮은 노드가 primary인 상태를 방치하지 않는다.** 3노드는 두 DB 노드가 같은 사양이라 사양을 맞출 일이 없다.
...
**페일오버** — 0초는 `stop-instances` 명령을 낸 14:02:01.86

| 순서 | 명령 | 멈춤 조건 · 실측 |
|---|---|---|
| 1 | 페일오버 전 primary(DB-01)에 표식 행 P → standby(DB-02)에 도착 확인 | 리허설에서만 |
| 2 | APP-01 `bash maintenance.sh on` | 2.2초 |
...
- **승격된 DB-02는 논리 백업을 스스로 판정한다** — 스크립트가 primary 여부를 본다. 이번에 확인한 것은 8의 수동 실행까지이고, **02:00 timer가 DB-02에서 발화하는 것은 보지 않았다**
- DB-02의 5432는 평시에도 사설 IP에 게시돼 있어(시스템 구성서 4장) 승격 즉시 앱이 붙는다

**페일백** — 경과의 0초는 각 행에 적었다
...
| 2 | **멈춤 조건** — DB-02가 primary, 페일오버 뒤 쓴 표식 행(Q)이 있다, **페일오버 8의 논리 백업이 성공했다(S3)**(6.2 「2 앞」의 덤프와 같은 몫) → DB-01 `sudo find /srv/pgdata/data -mindepth 1 -delete` → `.env` `PG_BOOTSTRAP_FROM=10.20.20.10` → `docker compose up -d postgres` | 슬롯 없이 받는다(초기화 스크립트가 이어받은 슬롯 이름을 지운다). 명령 14:05:09.6 → `started streaming` 로그 14:05:13.2(3.5초) → healthy 확인 14:05:17.1(7.4초). DB-01에 P,Q |
| 3 | APP-01 점검 모드(0초 — 14:05:34.49) → DB-02 `SELECT pg_current_wal_lsn()` 기록 | — |
| 4 | DB-02 `docker compose stop postgres-standby` | **정지 뒤에 5를 본다** — 6.2 6행과 같은 순서. 기록과 정지 사이에 들어온 쓰기까지 잡힌다 |
| 5 | **멈춤 조건** — DB-01 `pg_last_wal_replay_lsn() >= <3의 값>` → DB-01 `SELECT pg_promote(true, 60)` | timeline +1, 표식 행 P,Q |
| 6 | APP-01 `.env` `DB_HOST=10.20.10.10` → 두 슬롯 · exporter 재생성 → **멈춤 조건** — 두 슬롯 healthy, DB-01의 `pg_stat_activity`에 `10.20.0.10` 연결 → 점검 모드 해제 | 아래 |
| 7 | **멈춤 조건** — DB-01이 primary · 표식 행 · 밖에서 200 → DB-01 `.env`에서 `PG_BOOTSTRAP_FROM` 삭제 → DB-01 `SELECT pg_create_physical_replication_slot('standby_1')` → DB-02 데이터 자리 비우기 → `docker compose up -d postgres-standby` | 명령 14:09:38.8 → `started streaming` 14:09:41.0(2.2초) → 확인 14:09:45.7(6.9초) · TLS · 슬롯 active |
| 8 | **DB-01에서 물리 백업을 바로 한 번**(`systemctl start rental-basebackup`) | 아래 둘째 |
| 9 | 두 노드 테이블 건수 대조 · 표식 테이블 삭제 | 같다 — 유실 없음 |

**이번 실행은 이 표와 두 군데가 달랐다**(#239).
...
| 용량 | 디스크 사용률 추이(노드별 데이터 볼륨 · S3 버킷 용량 — 옛 단일 노드는 NAS), 증설 판단 | 2장 일일 점검 기록 — 대시보드는 관측을 세우면 함께 |
...
- **전 노드에 같은 순서로** 적용한다. 한 노드라도 빠지면 노드마다 UID가 달라지고(자동 할당 — 아래 2) 감사 기록이 어긋난다.
...
**추가 명령** — 옛 AL2023 APP-01에 `buyeon`(운영자 개인) · `deploy`를 만든 순서다(2026-09-25 11:43 ~ 11:48, #229). 3노드는 같은 순서를 세 노드에 밟았다(9.1 「3노드」 6). **1 ~ 5는 그때 밟았고, 1의 비밀구절과 6은 사용자가 직접 할 일로 남아 있다** — 6의 확인 칸은 기대값이다.
...
| 확인 | 방법 |
|---|---|
| 마운트(옛 단일 노드만 — 3노드에는 NFS가 없다) | 클라이언트에서 `findmnt <백업 경로>` — NFSv4로 붙어 있는가 |
| 논리 백업이 쌓이는가 | 3노드는 primary DB 노드의 `/var/backups/rental`에 오늘 날짜 파일 · 크기, S3 사본은 운영자 자격 증명으로 `aws s3 ls s3://rental-backup-890742606734/logical/`(노드의 역할은 목록을 못 본다 — 5장). 옛 단일 노드는 NAS의 백업 경로 |
...
**설치 순서** — 옛 APP-01(Amazon Linux 2023, DB 컨테이너가 같은 노드)에 2026-09-24 00:35 ~ 15:01에 실제로 밟은 순서다. 1 ~ 9는 00:35 ~ 04:04에 적용했고 명령은 그 결과 상태(`id` · `stat` · `rpm` · dnf 이력 · 역할 조회)와 대조해 적었다 — 2는 01:22 첫 수동 실행이 실패한 뒤 더한 단계다. 10은 15:00 ~ 15:01. 3노드는 이 순서를 두 DB 노드에서 다시 밟았다 — 아래 「3노드(Rocky) 설치」.
...
**물리 백업 · WAL 아카이빙(`rental-basebackup`)** — 5장. 설치 순서는 아래다(#214). 논리 백업 설치를 마친 노드를 전제한다 — `backup` 계정 · `/etc/rental` · 게시(옛 단일 노드는 루프백, 3노드는 사설 IP)가 이미 있다.
...
**(옛 단일 노드) NAS가 멈추면** 서비스는 영향이 없다(시스템 구성서 5.2). 물리 백업 · WAL(3노드는 로컬 + S3 — 5장, 옛 단일 노드는 로컬)은 계속되므로 급하게 손대지 않고, 복구 뒤 멈춘 기간의 논리 백업을 한 번 수동 실행한다.
...
| 2 | `.env` — 옛 노드의 것을 **운영자 PC 파이프로 스트림**(`ssh 옛 'cat …/.env' \| ssh 새 'sudo -u deploy sh -c "umask 077; cat > …"'` — 이 PC 디스크에 남지 않는다) → 노드별 값: APP-01 `COMPOSE_PROFILES=app` · `DB_HOST=10.20.10.10` · `APP_BASE_URL` · `PG_EXPORTER_SSLMODE=require`, DB-01 `db` · `PG_PUBLISH_ADDR=10.20.10.10` · `PG_SSL=on` · `PG_DATA_SOURCE=/srv/pgdata/data`, DB-02 `standby` · `PG_STANDBY_FROM=10.20.10.10` · `PG_STANDBY_PUBLISH_ADDR=10.20.20.10` · `PG_STANDBY_PUBLISH_PORT=5432` · `PG_SSL=on` · `PG_STANDBY_DATA_SOURCE=/srv/pgdata/data`. **DB 노드는 그 노드가 쓰는 키만 남긴다**(#247, 2026-09-25 15:58) — `COMPOSE_PROFILES` · `POSTGRES_DB` · `POSTGRES_USER` · `POSTGRES_PASSWORD` · `POSTGRES_REPLICATION_USER` · `POSTGRES_REPLICATION_PASSWORD` · 그 노드의 `PG_*` · `RENTAL_NODE` · `GRAFANA_CLOUD_PROM_URL` · `_USER` · `_TOKEN`(DB-02의 `POSTGRES_PASSWORD`는 standby가 쓰지 않지만 `postgres` 서비스 정의가 기본값 없이 참조해 치환 경고를 막으려고 둔다). 앱 · 메일 · Redis · JWT · 로그 전송 · 모니터 계정 값은 APP-01에만 둔다 — 노드가 털렸을 때 새는 비밀을 줄인다. 줄인 뒤 `config` 경고 0 · `up -d --dry-run` 재생성 0을 본다 | `docker compose config --services`(`deploy`로) — APP-01 11 · DB-01 `postgres` · DB-02 `postgres-standby` |
