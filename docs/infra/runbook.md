...
**아직 정해지지 않은 것** — 목적지(지금은 노드의 로컬 경로, NAS는 INF-08 이후) · 보존 기간 · 암호화 도구 · 실행 시간 상한. 잠정값은 스크립트와 유닛이 갖고, 확정 시점은 서버 운영 기반 설계서 5.4 · 10장이 갖는다. **선행 조건이 하나 있다** — 지금 PostgreSQL은 APP-01의 컨테이너이고 호스트 포트를 열지 않아 호스트 클라이언트가 닿지 않는다. 루프백 게시(같은 설계서 10장 「5432 호스트 규칙(DOCKER-USER)」)가 정해져야 이 작업을 노드에 넣을 수 있다.
