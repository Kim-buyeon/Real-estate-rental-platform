...
| 노드 | 재부팅 중 | 방법 |
|---|---|---|
| APP-01 | **서비스 전체 중단**(단일 노드 — 두 슬롯 · Nginx가 함께 멈춘다, 시스템 구성서 3장). **실측 — 부팅 후 전 컨테이너 정상 88초**(2026-09-23). 2026-09-25 재부팅(SELinux enforcing 전환 확인)은 명령에서 API 첫 200까지 약 93초(9.1). 화면은 10초쯤에 뜨지만 그때 API 는 아직 실패한다 — **복귀 판정은 API 로 한다**(71초) | 공지 후 저트래픽 시간대. 기동 뒤 배포 확인(3장)과 같은 확인 |
