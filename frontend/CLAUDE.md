...
- **발급 실패도 같은 백오프로 다시 시도한다.** 재시도는 항상 타이머를 거친다 — 즉시 다시 부르는 경로를 두지 않는다. 지연은 회차마다 두 배로 벌어지되 상한(30초)에서 멈추고, **그 상한의 절반부터 상한 사이에서 난수로 정해진다.** 고정된 수열을 쓰면 서버가 복구되는 순간 모든 클라이언트의 재시도가 한 점에 몰린다. **멈추는 것은 401 · 403뿐이고**(다시 시도해도 결과가 같다) **그 밖의 실패는 멈추지 않고 계속 시도한다** — 서버가 복구되면 붙어야 하기 때문이다. 다시 로그인하면 이 컴포넌트가 새로 마운트되며 연결한다. 실시간 전달 실패는 기능 실패가 아니므로 화면에 오류로 띄우지 않는다.
