...
- **발급 실패도 같은 백오프로 다시 시도하되 무한히 하지 않는다.** 재시도는 항상 타이머를 거쳐 1초에서 30초로 벌어지고 — 즉시 다시 부르는 경로를 두지 않는다 — 다시 시도해도 결과가 같은 실패(401 · 403)에서는 멈춘다. 다시 로그인하면 이 컴포넌트가 새로 마운트되며 연결한다. 실시간 전달 실패는 기능 실패가 아니므로 화면에 오류로 띄우지 않는다.
