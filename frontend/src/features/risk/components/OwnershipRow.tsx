...
// 행 레이아웃은 목록(RegistryTimeline)의 것이다 — 갑구 · 을구 두 표의 라벨 열과 값 열이 같은
 ...
 *
 * 행 모양은 {components.kv-row}다. `<ol>` · `<li>` 목록이라 `<dt>` · `<dd>`를 쓰는 `KvRow` 대신
 * `kvRowClassName`을 입힌다 — 라벨 열 150에 권리 종류와 순위번호, 값 열에 명의와 원인, 접수일이다.
 */
export const OwnershipRow = memo(function OwnershipRow({ ownership }: OwnershipRowProps) {
  const rowClasses = ownership.isActive
  ...
  return (
    <li className={rowClasses}>
      <span className={styles.label}>
        ...
        <span className={`${styles.rank} type-body-sm`}>순위 {ownership.rankNo}</span>
      </span>
      <span className={styles.value}>
        ...
        <span className={`${styles.date} type-body-sm`}>{formatDate(ownership.receivedDate)}</span>
      </span>
    </li>
  );
});
