...
export function NotificationStream() {
  ...
  useEffect(() => {
     ...
     * 가져온다.
     *
     * **멈추는 것은 401 · 403뿐이다** — 세션이 유효하지 않다는 뜻이고(api/client.ts가 만료 토큰
     * 재발급을 이미 한 번 시도한 뒤다) 이 상태로 다시 발급받을 수는 없다. 다시 로그인하면 이
     * 컴포넌트가 새로 마운트되며 연결한다.
     *
     * **그 밖의 실패(500 · 네트워크)는 30초 간격으로 계속 시도한다.** 서버가 복구되면 붙어야
     * 하기 때문이다. 요청이 몰아치지는 않는다 — 재시도는 항상 백오프 타이머를 거쳐 상한이 1s · 2s ·
     * 4s … 30s로 벌어지고(그 상한의 절반~상한 사이에서 흩어진다, backoffDelay) 즉시 다시 부르는
     * 경로가 없다.
     */
