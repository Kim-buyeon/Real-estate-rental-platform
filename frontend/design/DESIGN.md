...
> 픽셀 배율은 ×1.0으로 확정했다 (`analysis/tokens.md` 0장). 이 문서의 px는 계측 원본 그대로다.
...
> (`home` · `listing`은 우리가 만들지 않는 화면이고 토큰 인벤토리 계측에만 썼다. `home/01` · `home/02`는
> 1.35배 확대 캡처라 보정값이며, 푸터 정본은 `home/03`이다.)
