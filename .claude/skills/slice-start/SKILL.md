---
name: slice-start
description: 저장소를 바꾸는 작업이나 운영 작업에 착수할 때 어느 문서를 읽을지 정한다. 기능 ID 또는 작업 내용을 받아 백엔드 기능·프론트 기능·인프라·기반·운영·문서 중 갈래를 판별하고, 그 갈래에 필요한 문서만 컨텍스트에 올린다.
---
...
**고칠 문서를 먼저 정하고, 그것을 인용하는 쪽을 찾아 함께 올린다.** 숫자 · 절 번호 · 경로는 복사되어 있지 않고 인용되어 있다. 한쪽만 고치면 인용이 낡는다.

| 올린다 | 고르는 방법 |
| --- | --- |
| 고칠 문서 | 이슈가 지목한 것 |
| 그것을 인용하는 문서 | 파일명으로 저장소 전체를 검색한다. `docs/` 밖의 `.claude/` · `.github/` · 루트 `CLAUDE.md`도 포함한다 |
| 인용된 절 | 인용이 절 번호까지 지목하면 그 절만. 문서 전체를 올리지 않는다 |

**영역 색인과 구현 방침은 거치지 않는다.** 무엇을 만드는지가 아니라 무엇이 적혀 있는지를 고치는 작업이다.

**고칠 문서가 기능 정의나 API 명세면 그 영역의 짝을 함께 올린다.** 둘은 같은 기능의 앞뒤이므로 한쪽만 보고 고치면 다른 쪽과 어긋난다.
...
기반 갈래는 **동결만 확인한다.** 차기 범위와 선행 기능은 기능에만 적용된다. **운영 · 문서 갈래는 3장을 건너뛴다.** 기능을 만드는 작업이 아니다.
...
| 대상 | 이유 |
| --- | --- |
| `docs/workflow.md` · `docs/git/` · `docs/conventions.md` | 루트 `CLAUDE.md`가 지시한다. **문서 갈래에서 이 문서들 자체를 고치는 경우는 예외다** |
