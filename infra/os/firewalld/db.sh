...
#       Docker 설치 뒤에 다시 돌릴 때 --reload 가 Docker 규칙에 주는 영향은 노드에서 확인하지 않았다 — 그때는 복제 · 앱 접속을 함께 본다.
#
# ── 이 노드가 받는 것 ──
#  - ssh — APP-01 을 거쳐 들어온다(ProxyJump). 출발지 제한은 보안 그룹(rental-db: 22 ← rental-app)이 한다.
#    노드별 허용 표는 시스템 구성서 4장.
#  - postgresql(5432) — 이 규칙은 **호스트 프로세스용**이다. PostgreSQL 을 Docker 가 게시하면 5432 는 firewalld 를 우회한다
#    (DNAT 이 입력 규칙보다 먼저 목적지를 바꾼다). 그 경우 이 포트를 지키는 것은 보안 그룹과 사설 서브넷 두 겹이다 —
#    설계서 3.3 「예외 — DB 노드의 5432」. 게시 주소를 노드 사설 IP 하나로 좁히는 것(0.0.0.0 금지)은 운영 Compose 의 몫이다.
#    DOCKER-USER 체인으로 한 겹 더 두지 않는다(2026-09-25 결정, 같은 절) — 보안 그룹이 이미 그룹 단위로 출발지를 좁혀
#    같은 출발지를 IP 로 한 번 더 적는 것뿐이라 좁혀지는 범위가 없다.
set -euo pipefail
